...
const Home: NextPage<props> = ({users}: props) => {
   ...
   const handleButtonClick = async (nickname: string): Promise<void> => {
      const response = await fetch('http://localhost:3000/api/hello', {method: 'POST', body: JSON.stringify(nickname)})
  ...
  return (
      <List
          ...
          footer={<Input.Group compact style={{ display: 'flex' }}>
              <Input placeholder='enter nickname' onChange={handleInput}/>
              <Button value={nickname} disabled={!nickname} type="primary" onClick={() => handleButtonClick(nickname)}>Join to lobby</Button>
          </Input.Group>}
          bordered
